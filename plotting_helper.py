...
## and colour palettes
## Taken from https://raw.githubusercontent.com/HEP-KBFI/CombineHarvester/master/CombineTools/python/plotting.py
##@{
...
def makeHist1D(name, xbins, graph, scalerange=1.0):
    len_x = graph.GetX()[graph.GetN() - 1] - graph.GetX()[0]
    binw_x = (len_x * 0.5 / (float(xbins) - 1.)) - 1E-5
    hist = R.TH1F(
        name, '', xbins, graph.GetX()[0], scalerange * (graph.GetX()[graph.GetN() - 1] + binw_x))
    return hist
...
def RemoveInrange(gr, xmin=0, xmax=1):
    remove_list = []
    for i in range(gr.GetN()):
        if gr.GetY()[i] == 0.:
            continue
